...
plan: 01
subsystem: packaging
tags: [pyproject, pydantic, validation, rich, cli, testing]

# Dependency graph
requires:
  - phase: 05-cli-integration
    provides: CLI entry points (run_screener.py, run_strategy.py) and config_loader with ValidationError
provides:
...
tech-stack:
  added: []
  patterns:
    - "try/except ValidationError with Rich Panel display in CLI entry points"
    - "monkeypatch dotenv.load_dotenv before importlib.reload for test isolation"

key-files:
  created:
    - tests/test_cli_screener.py::test_config_error_shows_panel
    - tests/test_cli_strategy.py::test_config_error_shows_panel_with_screen
  modified:
    - pyproject.toml
